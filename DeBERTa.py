...
# Cargar modelo y tokenizador DeBERTa
model_name = "microsoft/deberta-base"
model = DebertaForSequenceClassification.from_pretrained(model_name, num_labels=5)
tokenizer = DebertaTokenizer.from_pretrained(model_name)

# Congelar capas si se desea (opcional)
for name, param in model.deberta.encoder.layer[:10].named_parameters():
...
# Tokenización
train_encodings = tokenizer(train_texts.tolist(), truncation=True, padding=True, max_length=75)
test_encodings = tokenizer(test_texts.tolist(), truncation=True, padding=True, max_length=75)
...
plt.ylabel('Real')
plt.title('Matriz de Confusión DeBERTa')
plt.tight_layout()
plt.show()
