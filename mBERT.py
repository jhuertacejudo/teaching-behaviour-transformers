...
# Congelar las primeras 6 capas del encoder (opcionalmente puedes ajustar el rango)
...
train_encodings = tokenizer(train_texts.tolist(), truncation=True, padding=True, max_length=456)
test_encodings = tokenizer(test_texts.tolist(), truncation=True, padding=True, max_length=456)
...
# Optimización
optimizer = AdamW(model.parameters(), lr=1.1991760380369865e-05, weight_decay=0.1979580541296286)
...
plt.ylabel('Real')
plt.title('Matriz de Confusión BERT Multilingual')
plt.tight_layout()
plt.show()
